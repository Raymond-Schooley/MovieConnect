...
#MUST CHANGE THESE
imdbFile = 'C:\\Users\\...\\title.basics.tsv'
#full path or sql command (ex: mysql)
sqlLocation = 'C:\\Users\\...\\mysql.exe'
sqlUser = 'root'
sqlPass = '.....'
...
with open(imdbFile, encoding="utf8") as tsvfile:
    ...
    for row in reader:
        ...
        if rowCount % 100 == 0:
            print("%s movies kept/added out of %s total movies --- file progress: %.4f %%" % (rowsAdded, rowCount, (rowCount/4783534)*100) )
        ...
        # and throw out rows that don't have certain fields
        tconst = row[0]
        ...
        sqlCommand = sqlPrefix + "\"INSERT INTO imdb.movies_info_raw VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s');\"" % (tconst,
            primaryTitle, originalTitle, isAdult, startYear, runtimeMinutes, genres)
        os.system(sqlCommand)
        rowsAdded += 1

print("Total rows added to table: %s" % rowsAdded)
